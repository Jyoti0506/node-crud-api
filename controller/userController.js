...
const createUser =async (req,res) => {
   try {
        ...
        res.status(StatusCodes.OK).json({msg: "New User created suceessfully", user: newUser })
   } catch (err) {
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ msg: err })
}
}
...
const deleteUser = async (req,res) => {
   try {
        ...
        let id = req.params.id
            let extUser = await User.findById({ _id: id })
            if(!extUser)
                return res.status(StatusCodes.NOT_FOUND).json({ masg: `Requested id not found`})

            await User.findByIdAndDelete({ _id: id })

    res.status(StatusCodes.OK).json({msg: "User delete successfully"})
   } catch (err) {
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ msg: err })
} 
}
